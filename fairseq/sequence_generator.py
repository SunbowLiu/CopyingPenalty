    ...
    def _generate(
        ...
        for step in range(max_len + 1):  # one extra step for EOS marker
            ...
            lprobs, avg_attn_scores = self.model.forward_decoder(
                tokens[:, : step + 1], cp, encoder_outs, self.temperature,
            )
...
class EnsembleModel(nn.Module):
    ...
    @torch.jit.export
    def forward_decoder(
        self, tokens, cp, encoder_outs: List[EncoderOut], temperature: float = 1.0,
    ):
        log_probs = []
        ...
        for i, model in enumerate(self.models):
            ...
            # add copying penalty
            decopy_src = encoder_outs[i].src_tokens.unsqueeze(1).expand(-1, probs.size(1), -1)
            ...
            decopying_penalty = torch.log(torch.tensor(cp['alpha']))
